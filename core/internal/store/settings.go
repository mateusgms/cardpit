...
const (
	...
	SetTelegramChatIDsSrc = "telegram_chat_ids_source" // TokenSourceEnv | TokenSourceUI
	SetAPIToken           = "api_token"                // sealed
	SetAutoUpdate         = "auto_update"              // "true" | "false", default true
)

// Values for SetTelegramTokenSrc and SetTelegramChatIDsSrc. A value seeded
// from the TELEGRAM_KEY/TELEGRAM_CHAT_ID env vars (TokenSourceEnv) is
// re-seeded on every boot so rotating the variable propagates; once the UI
// writes a value (TokenSourceUI) the env var no longer touches it.
const (
	TokenSourceEnv = "env"
	TokenSourceUI  = "ui"
)
